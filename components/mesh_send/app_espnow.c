...
extern char Device_payload[ESPNOW_PAYLOAD_MAX_LEN];

static const char *TAG = "app_espnow";
static uint32_t current_seq = 0;
static TaskHandle_t Device_handle = NULL;
static QueueHandle_t espnow_recv_queue = NULL;
static SemaphoreHandle_t sent_msgs_mutex = NULL;
static int i = 0;
static int j = 0;

// Define a global variable to track the last sent sequence number
...
// Define variables to store the last received sequence number for each data type
uint32_t last_seq_temperature = 0;
uint32_t last_seq_humidity = 0;

/* Parse received ESPNOW data. */
esp_err_t espnow_data_parse(uint8_t *data, uint16_t data_len, uint32_t *seq, float *value)
{
    ...
    *seq = buf->seq;
    if (buf->mesh_id == esp_mesh_lite_get_mesh_id()) {
        switch (buf->sensor_data.data_type) {
        case SENSOR_TEMPERATURE:
            *value = buf->sensor_data.payload.temperature;
            if (*seq == last_seq_temperature) {
                ESP_LOGI(TAG, "Duplicate message received (Seq: %d), ignoring.", *seq);
                return ESP_FAIL;
            }
            last_seq_temperature = *seq; // Update last received sequence number
            global_temperature = *value; // Store temperature value in global variable
            break;
        case SENSOR_HUMIDITY:
            *value = buf->sensor_data.payload.humidity;
             if (*seq == last_seq_humidity) {
                ESP_LOGI(TAG, "Duplicate message received (Seq: %d), ignoring.", *seq);
                return ESP_FAIL;
            }
            last_seq_humidity = *seq; // Update last received sequence number
            global_humidity = *value; // Store temperature value in global variable
            break;
        // Add cases for additional sensor data types here as needed
        default:
            // Unsupported sensor data type
            return ESP_FAIL;
        }
        return ESP_OK;
    }
...
void espnow_data_prepare(uint8_t *buf, float value, sensor_type_t data_type, bool seq_init)
{
    ...
    switch (data_type) {
        case SENSOR_TEMPERATURE:
            temp->sensor_data.data_type = SENSOR_TEMPERATURE;
            temp->sensor_data.payload.temperature = value;
            break;
        case SENSOR_HUMIDITY:
            temp->sensor_data.data_type = SENSOR_HUMIDITY;
            temp->sensor_data.payload.humidity = value;
            break;
        // Add cases for additional sensor data types here as needed
        default:
            break;
...
{
    i++;
    size_t payload_len = sizeof(app_espnow_data_t);
    uint8_t *buf = calloc(1, payload_len + ESPNOW_PAYLOAD_HEAD_LEN);
    espnow_data_prepare(buf, value, data_type, seq_init);
    ...
    while (xQueueReceive(espnow_recv_queue, &evt, portMAX_DELAY) == pdTRUE) {
        switch (evt.id) {
            case ESPNOW_RECV_CB:
            {
                ...
                if (espnow_data_parse(recv_cb->data, recv_cb->data_len, &recv_seq, &received_value) != ESP_OK) {
                    // Error parsing received data
                    goto cleanup;
                }
                cJSON *light_js = NULL;
                cJSON *rmaker_data_js = cJSON_Parse((const char*)Device_payload);
                if (rmaker_data_js) {
...
esp_err_t message_init(void)
{
    ...
    xTaskCreate(espnow_task, "espnow_task", 3 * 1024, NULL, 4, &Device_handle);
